...
USERS = {
    ...
    "3jiwon": {"password": "incar33**", "name": "3지원센터 관리자", "dept": "ALL", "role": "admin"}
}
...
def render_dashboard():
    ...
    # -------------------------------------------------------------
    with tab1:
        st.markdown("### 🔍 이름 또는 사원번호로 검색해주세요")
...
* **동양, 처브**는 서면위촉 보험사이므로, 미해촉 혹은 미제출시 위촉서류 원본을 지원센터로 발송해주세요.
* **KB, 신한, 메트라이프, 하나생명**은 고정링크 선동의보험사로 FA님께 제출여부 확인하시어 접수바랍니다.
...
""")
        ...
        with st.form("request_form", clear_on_submit=True):
            ...
            # 최대 5개 선택 가능한 보험사 다중 선택 (서면위촉 보험사 2개 제외)
            insurance_options = [
                "ABL생명", "DB생명", "IBK연금", "iM라이프", "KB라이프생명", "KDB생명", 
                "NH농협생명", "교보생명", "라이나생명", "메트라이프", "미래에셋", 
